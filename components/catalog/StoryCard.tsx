...
//
// Desktop adds a Prime Video-style floating hover card after a 400ms delay
// (mounted via portal to escape Row overflow). Mobile keeps the bare Link
// → /s/{slug} flow — tap navigates instantly.
...
export function StoryCard({
  ...
  /**
   * Whether the current viewer has this story in their My List. Used to
   * initialize the toggle button inside the hover card.
   */
  isFavorite?: boolean;
}) {
  ...
  // Close the hover-card if the user scrolls (the absolute coords go stale
  // anyway, and Netflix/Prime behave the same way).
  useEffect(() => {
    if (!anchor) return;
    const close = () => {
